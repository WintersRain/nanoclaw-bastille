...
This isn't a framework or a platform. It's working software for my specific needs. I use Discord, so it supports Discord. I don't use Telegram, so it doesn't support Telegram. I add the integrations I actually want, not every possible integration.
...
**Core components:**
- **Google Gemini API** (`@google/genai`) as the core agent
...
- Use existing tools (Discord connector, Gemini API, function calling)
- Minimal glue code
...
### Session Management
- Each group maintains a conversation session (via Gemini agent runner)
...
### Discord
- Using discord.js library for Discord gateway connection
- Messages stored in SQLite, polled by router
- Bot token authentication during setup

### Scheduler
- Built-in scheduler runs on the host, spawns containers for task execution
- Agent runner provides scheduling tools via function calling
- Tools: `schedule_task`, `list_tasks`, `pause_task`, `resume_task`, `cancel_task`, `send_message`
- Tasks stored in SQLite with run history
- Scheduler loop checks for due tasks every minute
- Tasks execute Gemini agent runner in containerized group context

### Web Access
- Built-in web_fetch tool and Google Search via native Gemini tool
...
### Skills
- `/setup` - Install dependencies, configure Discord bot, configure scheduler, start services
...
- **Main channel**: Admin channel (private Discord channel)
